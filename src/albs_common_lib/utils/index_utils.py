"""
AlmaLinux Build System repositories management utilities.
"""
...
def extract_metadata(rpm_file, txn=None, checksum=None):
    ...
    for f in ('obsoletes', 'provides', 'conflicts'):
        for name, flag, (epoch, ver, rel), _ in get_rpm_property(hdr, f):
            ...
            if data not in meta[f]:
                meta[f].append(data)
    for name, flag, (epoch, ver, rel), pre in get_rpm_property(
        hdr, 'requires'
    ):
        data = {'name': to_unicode(name)}
        if flag is not None:
    ...
    if hdr[rpm.RPMTAG_EXCLUDEARCH]:
        meta['excludearch'] = [
            to_unicode(arch) for arch in hdr[rpm.RPMTAG_EXCLUDEARCH]
        ]
