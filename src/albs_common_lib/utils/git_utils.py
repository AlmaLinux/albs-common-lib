"""
AlmaLinux Build System git wrapper.
"""
...
from albs_common_lib.errors import CommandExecutionError, LockError

from .file_utils import download_file, safe_mkdir
from .ported import to_unicode
from .rpm_utils import string_to_version as stringToVersion
...
def git_get_commit_id(repo_path, ref='HEAD'):
    ...
    exit_code, stdout, stderr = (
        git.with_cwd(repo_path)
        .with_env(HISTFILE='/dev/null', LANG='C')
        .run(('log', '--pretty=format:%H', '-n', 1, ref), retcode=None)
    )
    return stdout.strip()
...
def git_ls_remote(repo_path, heads=False, tags=False):
    ...
    are omitted.
    """
    parsed_path = urllib.parse.urlparse(repo_path)
    url_scheme = parsed_path.scheme
    git = plumbum.local['git']
    args = ['ls-remote']
    if heads:
        args.append('--heads')
    if tags:
        args.append('--tags')
    if url_scheme in ['http', 'https', 'ssh', 'git', 'whatever']:
        args.append(repo_path)
        exit_code, stdout, stderr = git.with_env(
            HISTFILE='/dev/null', LANG='C'
        )[args].run(retcode=None)
    else:
        exit_code, stdout, stderr = (
            git[args]
            .with_env(HISTFILE='/dev/null', LANG='C')
...
def list_git_tags(uri, commit_id=False):
    ...
    @raise GitError: When command execution failed.
    """
    return [
        (commit, ref) if commit_id else ref
...
def parse_cl_git_tag(tag):
    """
    Alma Linux git tags ([name@][epoch+]version[-release][^modifier]) parsing
    function.
    ...
    if not re_rslt:
        raise ValueError("invalid Alma Linux git tag ({0}) format".format(tag))
    t = {}
...
def cmp_cl_git_tags(tag1, tag2):
    """
    Alma Linux git tags ([name@][epoch+]version[-release][^modifier])
    comparison function.
...
class WrappedGitRepo:
    ...
    @staticmethod
    def add_gerrit_commit_hook(repo_dir):
        hook_url = "https://gerrit.cloudlinux.com/tools/hooks/commit-msg"
        hook_path = os.path.join(repo_dir, ".git/hooks/commit-msg")
        download_file(hook_url, hook_path)
    ...
    def clone_from(
        ...
        @type depth:        int
        @param depth:       Amount of commits when cloning
        @type branch:       str
        @param branch:      Branch to clone
        @type attempts:     int
        @param attempts:    Number of retries to clone repository

        @raise GitError: If something went wrong.
        """
        cmd = ["git", "clone"]
        if depth:
        ...
        while attempts > 0:
            ...
            last_ret_code = git_proc.returncode
            if git_proc.returncode == 0:
                return
            else:
                # There's a special case when `depth` parameter is used:
                # - `depth` requires a default branch to be present;
                # - some repositories do not have default branch in them;
                # This leads to 255 exit code when cloning is performed,
                # but the process can be continued
                if depth and git_proc.returncode == 255:
                    return
                WrappedGitRepo.cleanup_on_error(repo_dir)
                attempts -= 1
                time.sleep(10)

        if attempts == 0 and last_ret_code != 0:
            raise GitError(
                "cannot clone {0} git repository to {1} "
                "directory: return code {2}, git output: {3}".format(
                    repo_url, repo_dir, last_ret_code, stdout
                )
            )
        ...
        for commit, raw_tag in raw_tags:
            try:
                ...
                if (
                    ...
                    parsed_tags.append(
                        AltGitTag(
                            raw_tag, None, None, None, None, None, commit
                        )
                    )
...
def git_submodule_init(git_sources_dir: str, logger=None):
    logger = logger or logging.getLogger('alt_git_repo')
    git_cmd = plumbum.local['git']
    args = ('submodule', 'init')

    exit_code, out, err = git_cmd.run(
        args=args, cwd=git_sources_dir, retcode=None
    )
    logger.debug('"git submodule init" output:\n%s', '\n'.join([out, err]))
    if exit_code != 0:
        raise GitError(
            f'Cannot init submodules, stdout: {out}\n' f'Stderr: {err}'
        )


def git_submodule_update(git_sources_dir: str, logger=None):
    logger = logger or logging.getLogger('alt_git_repo')
    git_cmd = plumbum.local['git']
    ...
    logger.debug('"git submodule update" output:\n%s', '\n'.join([out, err]))
    if exit_code != 0:
        raise GitError(
            f'Cannot update submodules, stdout: {out}\n' f'Stderr: {err}'
        )
