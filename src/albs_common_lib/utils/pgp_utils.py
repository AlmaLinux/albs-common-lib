"""AlmaLinux Build System PGP related utility functions."""
...
from ..constants import COMMUNITY_KEY_SUFFIX
...
class PGPPasswordDB():
    def __init__(
        ...
        if development_mode and not development_password:
            raise ConfigurationError(
                'You need to provide development PGP '
    ...
    def load_keys(
        self,
        ask_passwords: bool = False,
        list_secret_keys: bool = False,
    ):
        existent_keys = {
            key['keyid']: key for key in self.__gpg.list_keys(
                secret=list_secret_keys,
            )
        }
        for keyid in self.__key_ids_from_config:
            ...
            self.__key_ids[keyid]['keygrip'] = key['keygrip']
    ...
    def ask_for_passwords(self):
        ...
        errors.ConfigurationError
            ...
            incorrect.
        """
        self.load_keys(ask_passwords=True, list_secret_keys=True)
    ...
    def get_keygrip(self, keyid: str) -> str:
        return self.key_ids[keyid]['keygrip']
