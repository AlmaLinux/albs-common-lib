...
from albs_common_lib.errors import SpecParseError

from .ported import cmp, to_unicode
from .rpm_utils import flag_to_string, string_to_version
...
class RPMChangelogRecord(
    ...
    def __str__(self):
        header = '* {0} {1}'.format(
            self.date.strftime('%a %b %d %Y'), self.packager
        )
        return '{0}\n{1}'.format(header, '\n'.join(self.text))
...
class PackageFeature(
    collections.namedtuple(
        "PackageFeature",
        ["name", "flag", "evr", "epoch", "version", "release"],
    )
):
...
class ChangelogRecord(
    collections.namedtuple("ChangelogRecord", ["date", "packager", "text"])
):
...
class RPMHeaderWrapper():
    ...
    @property
    def name(self):
        return none_or_unicode(self._hdr[rpm.RPMTAG_NAME])
    ...
    @property
    def version(self):
        return none_or_unicode(self._hdr[rpm.RPMTAG_VERSION])

    @property
    def release(self):
        return none_or_unicode(self._hdr[rpm.RPMTAG_RELEASE])

    @property
    def evr(self):
        return none_or_unicode(self._hdr[rpm.RPMTAG_EVR])

    @property
    def summary(self):
        return none_or_unicode(self._hdr[rpm.RPMTAG_SUMMARY])

    @property
    def description(self):
        return to_unicode(self._hdr[rpm.RPMTAG_DESCRIPTION])

    @property
    def license(self):
        return none_or_unicode(self._hdr[rpm.RPMTAG_LICENSE])

    @property
    def vendor(self):
        return none_or_unicode(self._hdr[rpm.RPMTAG_VENDOR])

    @property
    def group(self):
        return none_or_unicode(self._hdr[rpm.RPMTAG_GROUP])

    @property
    def url(self):
        return none_or_unicode(self._hdr[rpm.RPMTAG_URL])

    @property
    def provides(self):
        return self.__read_package_features(
    ...
    def changelogs(self):
        changelogs = []
        for packager, date, text in sorted(
            zip(
                self._hdr[rpm.RPMTAG_CHANGELOGNAME],
    ...
    def __read_package_features(self, name_tag, flags_tag, version_tag):
        features = []
        for name, flag, evr in zip(
            self._hdr[name_tag], self._hdr[flags_tag], self._hdr[version_tag]
        ):
            ...
            features.append(
                PackageFeature(
                    to_unicode(name), flag, evr, e, v, none_or_unicode(r)
                )
            )
        return features
...
class SrcRPMHeaderWrapper(RPMHeaderWrapper):
    ...
    def __init__(self, hdr, sources):
        ...
        for name, pos, type_ in sorted(
            sources, key=cmp_to_key(lambda a, b: (a[1] > b[1]) - (a[1] < b[1]))
        ):
            ...
            if type_ == rpm.RPMBUILD_ISSOURCE:
                ...
                raise NotImplementedError(
                    "unsupported source type {0!r}".format(type_)
                )
...
class SpecParser():
    ...
    def __init__(self, spec_file, macros=None):
        ...
        try:
            ...
            rpm.reloadConfig()
        self.__source_package = SrcRPMHeaderWrapper(
            self.__spec.sourceHeader, sources=self.__spec.sources
        )
